...
from datasets import Dataset as Dataset_tf
import numpy as np
import json
import random
...
import argparse 
from utils import get_nl_dataset, get_llm_model, dict_to_namespace, get_nl_dataset_labels, get_nl_dataset_umls_codex
...
def main(params):
    if params.do_train: 
        ...
        if params.data_path == 'FB15k-237':
            with open(os.path.join(params.data_path, "relations.dict")) as fin:
                rel2id = dict()
                for line in fin:
                    rid, relation = line.strip().split('\t')
                    rel2id[relation] = int(rid)
            with open(os.path.join(params.data_path, "entities.dict")) as fin:
                ent2id = dict()
                for line in fin:
                    eid, entity = line.strip().split('\t')
                    ent2id[entity] = int(eid)
            train_file_name = f'train_sampled_{params.prob}_drop_dup' if params.do_sampled else 'train_drop_dup'
            if not os.path.isfile(os.path.join(params.data_path, params.llm_model_name, params.path_exp_name, str(params.path_checkpoint),f'{train_file_name}.pth')):
                logging.info(f"Train data preprocessing start")
                b_start_time = time.time()
                train_data = get_nl_dataset(data_path = params.data_path, exp_name = params.path_exp_name, checkpoint = params.path_checkpoint, mode='train_sampled' if params.do_sampled else 'train', params = params, prob = params.prob, rel2id = rel2id, ent2id = ent2id, tokenizer = tokenizer)
                ...
                logging.info(f"Start saved train data loading")
                b_start_time = time.time()
                train_data = get_nl_dataset_labels(data_path = params.data_path, exp_name = params.path_exp_name, checkpoint = params.path_checkpoint, mode='train_sampled' if params.do_sampled else 'train', prob = params.prob, ent2id = ent2id)
                train_dataset = TrainDataset(train_data, tokenizer = tokenizer, ent2id = ent2id, rel2id = rel2id, negative_sample_size = params.negative_sample_size, max_tokens = params.max_tokens, params = params)
                end_time = time.time() - b_start_time
                logging.info(f"Saved train data loaded, time: {end_time}")
            if params.non_valid:
                valid_dataset = None
            else:
                if not os.path.isfile(os.path.join(params.data_path, params.llm_model_name, params.path_exp_name, str(params.path_checkpoint), 'valid.pth' if not params.drop_dup else 'valid_drop_dup.pth')):
                    logging.info(f"Valid data preprocessing start")
                    b_start_time = time.time()
                    valid_data = get_nl_dataset(data_path = params.data_path, exp_name = params.path_exp_name, checkpoint = params.path_checkpoint, mode='valid', params = params, rel2id = rel2id, ent2id = ent2id, tokenizer = tokenizer)
                    ...
                    logging.info(f"Start saved valid data loading")
                    b_start_time = time.time()
                    valid_data = get_nl_dataset_labels(data_path = params.data_path, exp_name = params.path_exp_name, checkpoint = params.path_checkpoint, mode='valid', prob = params.prob, ent2id = ent2id)
                    valid_dataset = TestDataset(valid_data, tokenizer = tokenizer, ent2id = ent2id, rel2id = rel2id, max_tokens = params.max_tokens, params = params, mode='valid')
                    end_time = time.time() - b_start_time
                    logging.info(f"Saved valid data loaded, time: {end_time}")
            #"""
        elif params.data_path.split("/")[-1] in ['UMLS', 'CODEX']:
            ...
            logging.info(f"Train data preprocessing start")
            b_start_time = time.time()
            train_data = get_nl_dataset_umls_codex(data_path = params.data_path, exp_name = params.path_exp_name, checkpoint = params.path_checkpoint, mode='train_sampled' if params.do_sampled else 'train', params = params, prob = params.prob, rel2id = rel2id, ent2id = ent2id, tokenizer = tokenizer)
            train_dataset = TrainDataset(train_data, tokenizer = tokenizer, ent2id = ent2id, rel2id = rel2id, negative_sample_size = params.negative_sample_size, max_tokens = params.max_tokens, params = params)
            end_time = time.time() - b_start_time
            logging.info(f"Train data constructing done, time: {end_time}")
            ...
            logging.info(f"Valid data preprocessing start")
            b_start_time = time.time()
            valid_data = get_nl_dataset_umls_codex(data_path = params.data_path, exp_name = params.path_exp_name, checkpoint = params.path_checkpoint, mode='valid', params = params, rel2id = rel2id, ent2id = ent2id, tokenizer = tokenizer)
            valid_dataset = TestDataset(valid_data, tokenizer = tokenizer, ent2id = ent2id, rel2id = rel2id, max_tokens = params.max_tokens, params = params, mode='valid')
            end_time = time.time() - b_start_time
            logging.info(f"Valid data constructing done, time: {end_time}")
        else:
    ...
    if params.do_test_new:
        ...
        if saved_params.data_path.split('/')[-1] == 'FB15k-237':
            with open(os.path.join(params.data_path, "relations.dict")) as fin:
                rel2id = dict()
                for line in fin:
                    rid, relation = line.strip().split('\t')
            ...
            if not os.path.isfile(os.path.join(saved_params.data_path, saved_params.llm_model_name, saved_params.path_exp_name, str(saved_params.path_checkpoint),f'test.pth' if not params.drop_dup else 'test_drop_dup.pth')):
                b_start_time = time.time()
                test_data = get_nl_dataset(data_path = saved_params.data_path, exp_name = saved_params.path_exp_name, checkpoint = saved_params.path_checkpoint, mode='test', params=saved_params, rel2id = rel2id, ent2id = ent2id, tokenizer = tokenizer)            
                ...
                logging.info(f"Start saved test data loading")
                b_start_time = time.time()
                test_data = get_nl_dataset_labels(data_path = params.data_path, exp_name = params.path_exp_name, checkpoint = params.path_checkpoint, mode='test', prob = params.prob, ent2id = ent2id)
                test_dataset = TestDataset(test_data, tokenizer = tokenizer, ent2id = ent2id, rel2id = rel2id, max_tokens = saved_params.max_tokens, params= saved_params, mode='test')
                end_time = time.time() - b_start_time
                logging.info(f"Saved test data loaded, time: {end_time}")
        elif saved_params.data_path.split("/")[-1] in ['UMLS', 'CODEX']:
            ...
            logging.info(f"Test data preprocessing start")
            b_start_time = time.time()
            test_data = get_nl_dataset_umls_codex(data_path = saved_params.data_path, exp_name = saved_params.path_exp_name, checkpoint = saved_params.path_checkpoint, mode='test', params = saved_params, rel2id = rel2id, ent2id = ent2id, tokenizer = tokenizer)
            test_dataset = TestDataset(test_data, tokenizer = tokenizer, ent2id = ent2id, rel2id = rel2id, max_tokens = saved_params.max_tokens, params = saved_params, mode='test')
            end_time = time.time() - b_start_time
            logging.info(f"Test data constructing done, time: {end_time}")
        else:
            raise NotImplementedError
        if saved_params.data_path.split("/")[-1] in ['UMLS']:
            pass
        else:
            trainer = Trainer(model = best_model, params = saved_params, train = None, valid = None, test = test_dataset)
            trainer.test_new()
            if saved_params.data_path.split("/")[-1] == 'FB15k-237':
                if not os.path.isfile(os.path.join(saved_params.data_path, saved_params.llm_model_name, saved_params.path_exp_name, str(saved_params.path_checkpoint),f'test_hard.pth' if not params.drop_dup else 'test_hard_drop_dup.pth')):
                    b_start_time = time.time()
                    test_data = get_nl_dataset(data_path = saved_params.data_path, exp_name = saved_params.path_exp_name, checkpoint = saved_params.path_checkpoint, mode='test_hard', params=saved_params, rel2id = rel2id, ent2id = ent2id, tokenizer = tokenizer)            
                    ...
                    logging.info(f"Start saved test data loading")
                    b_start_time = time.time()
                    test_data = None#get_nl_dataset_labels(data_path = params.data_path, exp_name = params.path_exp_name, checkpoint = params.path_checkpoint, mode='test_hard', prob = params.prob, ent2id = ent2id)
                    test_dataset = TestDataset(test_data, tokenizer = tokenizer, ent2id = ent2id, rel2id = rel2id, max_tokens = saved_params.max_tokens, params= saved_params, mode='test_hard')
                    end_time = time.time() - b_start_time
                    logging.info(f"Saved test data loaded, time: {end_time}")
            elif saved_params.data_path.split("/")[-1] in ['CODEX']:
                
                logging.info(f"Test data preprocessing start")
                b_start_time = time.time()
                test_data = get_nl_dataset_umls_codex(data_path = saved_params.data_path, exp_name = saved_params.path_exp_name, checkpoint = saved_params.path_checkpoint, mode='test_hard', params = saved_params, rel2id = rel2id, ent2id = ent2id, tokenizer = tokenizer)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--use_scheduler', choices=['cosine', 'cosine_annealing', 'not_use'], default = 'cosine')
    ...
    if params.do_test and params.do_train:
