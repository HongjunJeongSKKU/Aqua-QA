...
def index_dataset(dataset_name, force=False):
    ...
    indexified_files = [
        "train_indexified.txt",
        "valid_indexified.txt",
        "test_indexified.txt",
    ]
    # files = ['train.txt']
...
def write_links(dataset, ent_out, small_ent_out, max_ans_num, name):
    ...
    for ent in ent_out:
        for rel in ent_out[ent]:
            if len(ent_out[ent][rel]) <= max_ans_num:
                ...
                tp_answers[(ent, (rel,))] = small_ent_out[ent][rel]  # easy
                fn_answers[(ent, (rel,))] = ent_out[ent][rel]  # hard
            else:
                num_more_answer += 1
    ...
    with open("./data/%s/%s-tp-answers.pkl" % (dataset, name), "wb") as f:
        pickle.dump(tp_answers, f)  # easy answer
    with open("./data/%s/%s-fn-answers.pkl" % (dataset, name), "wb") as f:
        pickle.dump(fn_answers, f)  # hard answer
    ...
    while num_sampled < gen_num:
        ...
        print(
            ...
            % (
                ...
                num_sampled,
                gen_num,
                (time.time() - s0) / (num_sampled + 0.001),
                num_try,
                num_repeat,
                num_more_answer,
                num_broken,
        ...
        # answer = random.sample(ent_in.keys(), 1)[0] # 원본 코드
        answer = random.sample(list(ent_in.keys()), 1)[0]  # 수정 코드

        broken_flag = fill_query(
        ...
        if len(answer_set) == 0:
            ...
            continue
        if mode != "train":
            if len(answer_set - small_answer_set) == 0:  # hard answer가 없는 경우 거름
            ...
            if "n" in query_name:
                if len(small_answer_set - answer_set) == 0:
                    ...
                    continue
        if (
            max(len(answer_set - small_answer_set), len(small_answer_set - answer_set))
            > max_ans_num
        ):
            num_more_answer += 1
            continue
        ...
        # if len(small_answer_set) == 0:
        #     print("I got you!!!!!!!!!!!!!!!!")

    print()
    logging.info(
        "{} tp max: {}, min: {}, mean: {}, std: {}".format(
            mode,
            np.max(tp_ans_num),
            np.min(tp_ans_num),
            np.mean(tp_ans_num),
...
def generate_queries(
    ...
    if gen_train or gen_valid:
        train_ent_in, train_ent_out = construct_graph(
            base_path, indexified_files[:1]
        )  # ent_in
    if gen_valid or gen_test:
        valid_ent_in, valid_ent_out = construct_graph(base_path, indexified_files[:2])
        valid_only_ent_in, valid_only_ent_out = construct_graph(
            base_path, indexified_files[1:2]
        )
    if gen_test:
        test_ent_in, test_ent_out = construct_graph(base_path, indexified_files[:3])
        test_only_ent_in, test_only_ent_out = construct_graph(
            base_path, indexified_files[2:3]
        )
    ...
    for i in range(len(query_structures)):  # for문 추가
        idx = i
        ...
        num_sampled, num_try, num_repeat, num_more_answer, num_broken, num_empty = (
            0,
            0,
            0,
            0,
        ...
        if gen_valid:
            valid_queries, valid_tp_answers, valid_fp_answers, valid_fn_answers = (
                ground_queries(
                    dataset,
                    query_structure,
                    valid_ent_in,
        ...
        if gen_test:
            test_queries, test_tp_answers, test_fp_answers, test_fn_answers = (
                ground_queries(
                    dataset,
                    query_structure,
                    test_ent_in,
...
def fill_query(query_structure, ent_in, ent_out, answer, ent2id, rel2id):
    ...
    if all_relation_flag:
        ...
        for i in range(len(query_structure[-1]))[::-1]:
            ...
            found = False
            for j in range(40):
                # r_tmp = random.sample(ent_in[answer].keys(), 1)[0] # 원본 코드
                r_tmp = random.sample(list(ent_in[answer].keys()), 1)[0]  # 수정 코드
            ...
            # answer = random.sample(ent_in[answer][r], 1)[0] # 원본 코드
            answer = random.sample(list(ent_in[answer][r]), 1)[0]  # 수정 코드
        if query_structure[0] == "e":
            query_structure[0] = answer
        else:
            return fill_query(
                query_structure[0], ent_in, ent_out, answer, ent2id, rel2id
            )
    else:
        ...
        for i in range(1, len(query)):
            if not union_flag:
                ent_set = ent_set.intersection(
                    achieve_answer(query[i], ent_in, ent_out)
                )
            else:
                if i == len(query) - 1:
                    continue
...
@click.command()
@click.option("--dataset", default="codex-s")
...
def main(
    ...
    if gen_train:
        ...
        for query_name in query_names:
            query = pd.read_pickle(f"data/{dataset}/{mode}-{query_name}-queries.pkl")
            df[list(query.keys())[0]] = list(query.values())[0]

        with open(f"data/{dataset}/{mode}-queries.pkl", "wb") as f:
            pickle.dump(df, f)

        hard_df = {}
        for query_name in query_names:
            hard_df.update(
                pd.read_pickle(f"data/{dataset}/{mode}-{query_name}-fn-answers.pkl")
            )
        with open(f"data/{dataset}/{mode}-answers.pkl", "wb") as f:
            pickle.dump(hard_df, f)
        ...
        hard_df = {}
        for query_name in query_names:
            easy_qa_set = pd.read_pickle(
                f"data/{dataset}/{mode}-{query_name}-tp-answers.pkl"
            )  # easy
            hard_qa_set = pd.read_pickle(
                f"data/{dataset}/{mode}-{query_name}-fn-answers.pkl"
            )  # hard (train의 경우는 easy, hard 상관없이 fn으로 사용)

            for k, v in easy_qa_set.items():
                easy_df[k] = v
                hard_df[k] = hard_qa_set[k]

        with open(f"data/{dataset}/{mode}-easy-answers.pkl", "wb") as f:
            pickle.dump(easy_df, f)

        with open(f"data/{dataset}/{mode}-hard-answers.pkl", "wb") as f:
            pickle.dump(hard_df, f)
    ...
    if dataset == "CODEX":
        with open(
            f"data/{dataset}/entities.json"
        ) as f:  # entity data codex/entities/en/entities.json
            entities = json.loads(f.read())

        ent2txt = {}
        for k, v in entities.items():
            if k in id2ent.values():
                ent2txt[k] = v["label"]
        with open(f"data/{dataset}/ent2txt.json", "w") as f:
            json.dump(ent2txt, f, indent=4)

        with open(
            f"data/{dataset}/relations.json"
        ) as f:  # relation data codex/relatons/en/relations.json
            relations = json.loads(f.read())
            id2rel_txt = {}

        for k, v in relations.items():
            if "+" + k in id2rel.values() or "-" + k in id2rel.values():
                id2rel_txt["+" + k] = "+" + v["label"]
                id2rel_txt["-" + k] = "-" + v["label"]

        with open(f"data/{dataset}/rel2txt.json", "w") as f:
            json.dump(id2rel_txt, f, indent=4)

    elif dataset == "UMLS":
        with open(f"data/{dataset}/entity2text.txt") as f:
            data = [line.strip().split("\t") for line in f]
            entities = {x[0]: x[1] for x in data}
        with open(f"data/{dataset}/ent2txt.json", "w") as f:
