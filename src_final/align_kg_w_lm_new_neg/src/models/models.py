...
class Ailgner_self_mask_modality(nn.Module):
    ...
    def forward(self, nl_query):
        ...
        #x = self.layers.layers[1](src = x, src_mask = ~expanded_mask_2)
    ...
    def train_step(self, nl_query, pos_ent, pos_rel, neg_ent, neg_rel, q_type_neg = None, q_type = None, type_to_id = None, rel_neg_indicator = None):
        ...
        #pos_rel[rel_neg_indicator] = self.neg_transform[pos_rel[rel_neg_indicator]]
        #neg_rel[rel_neg_indicator] = self.neg_transform[neg_rel[rel_neg_indicator]]
    ...
    def extract(self, nl_emb, attn_mask):
        ...
        expanded_mask = torch.cat([mask, nl_mask], dim = -1)
        expanded_mask = expanded_mask.unsqueeze(1).repeat(1, self.num_heads,1,1).view(-1, self.query_token.weight.size(0) + attn_mask.size(1), self.query_token.weight.size(0) + attn_mask.size(1))
        expanded_mask_2 = expanded_mask.clone().detach()
        expanded_mask_2[:,self.type_id] = 0
        expanded_mask_2[:,self.type_id, [self.type_id, self.and_id, self.or_id, self.not_id]] = 1

        x = self.layers.layers[0](src = inputs_embeds, src_mask = ~expanded_mask)
        x = self.layers.layers[1](src = x, src_mask = ~expanded_mask_2)
        #x = self.layers(src = inputs_embeds, mask = ~expanded_mask)
        ...
        return torch.cat([type_token, ent_token, rel_token, op_token], dim = 1)
    def extract_w_nl(self, nl_emb, attn_mask):
        ...
        return torch.cat([type_token, ent_token, rel_token, op_token], dim = 1), x[:,len(self.token_to_idx):,:]
