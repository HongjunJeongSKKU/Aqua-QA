...
class Trainer():
    def __init__(self, model, params, train = None, valid = None, test = None):
        ...
        if self.train_data is not None:
            ...
            elif self.params.use_scheduler == 'cosine_annealing':
                total_steps = math.ceil(len(self.train_data) / self.params.train_batch_size) * self.params.num_epochs
                warmup_steps = math.ceil(total_steps * self.params.warm_up_ratio)
                self.lr_scheduler = CosineAnnealingWarmupRestarts(self.optimizer, first_cycle_steps =total_steps, warmup_steps = warmup_steps, max_lr = self.params.lr, min_lr= self.params.lr * 0.01)
            else:
                pass
    ...
    def test(self):
        logging.info("Test start")
        metrics = self.validation(self.test_data)
        _, _ = self.evaluate(metrics, 'test')
        logging.info("Test finished")
    def validation_new(self, input_data):
        ...
        ############
        del all_query, all_query_padding_mask, paths, paths_padding_mask, neg_indicator, easy, hard, query_type#, pos_emb, adj
