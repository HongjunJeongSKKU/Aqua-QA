...
import time
from dataset import TrainDataset, TestDataset, TestDataset_original
...
def main(params):
    ...
    if params.do_train:
        ...
        start = time.time()
        params.max_length_input = 70#calculate_max_length(dataset = original_df, tokenizer = tokenizer)
        ...
        print(f"train data load finished {time.time() - start_time}")
        ...
        print(f"valid data load finished { time.time() - start_time}")
    ...
    if params.do_test:
        ...
        with open(os.path.join(params.output_directory, "params.json"), 'r') as f:
        ...
        with open("../data/rel2id.pkl", 'rb') as f:
            rel2id = pickle.load(f)
        ...
        aligner = Ailgner_self_mask_modality(params = params, llm_model = llm_model, kge_model = kge_model)

        aligner.load_state_dict(torch.load(os.path.join(saved_params.output_directory, 'best_model/best_model.pth' if not params.aligner_epoch else f"model_epoch_{params.aligner_epoch}.pth")), strict=False)
        aligner = aligner.to(saved_params.device)
